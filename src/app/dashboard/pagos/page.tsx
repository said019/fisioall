...
export default function PagosPage() {
  ...
  // KPIs
  const ingresosMes = mockPagos.filter(p => p.estado === "pagado").reduce((sum, p) => sum + p.monto, 0);
  const pendientesMes = mockPagos.filter(p => p.estado === "pendiente" || p.estado === "parcial").reduce((sum, p) => sum + p.monto, 0);
  const reembolsosMes = mockPagos.filter(p => p.estado === "reembolsado").reduce((sum, p) => sum + p.monto, 0);
  const totalTransacciones = mockPagos.length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Transacciones", valor: totalTransacciones.toString(), sub: "Total del mes", icon: FileText, color: "text-cyan-600", bg: "bg-cyan-50" },
        ].map((kpi) => (
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-baseline gap-2 mt-1">
                    <p className="text-2xl font-bold text-[#164E63]">{kpi.valor}</p>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex flex-col sm:flex-row gap-3 flex-1 w-full">
          ...
          <Select value={filtroEstado} onValueChange={setFiltroEstado}>
            <SelectTrigger className="w-full sm:w-40 border-cyan-200 h-10 cursor-pointer">
        ...
        <Button onClick={() => setModalRegistrar(true)} className="cursor-pointer bg-[#059669] hover:bg-[#059669]/90 text-white text-sm shadow-sm shrink-0">
      ...
      <Card className="border-cyan-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow className="bg-gradient-to-r from-[#ECFEFF] to-white">
                <TableHead className="text-[#164E63]/60 font-semibold text-xs">Paciente</TableHead>
                <TableHead className="text-[#164E63]/60 font-semibold text-xs">Concepto</TableHead>
                <TableHead className="text-[#164E63]/60 font-semibold text-xs text-right">Monto</TableHead>
                <TableHead className="text-[#164E63]/60 font-semibold text-xs">Método</TableHead>
                <TableHead className="text-[#164E63]/60 font-semibold text-xs">Estado</TableHead>
                <TableHead className="text-[#164E63]/60 font-semibold text-xs">Fecha</TableHead>
                <TableHead className="text-[#164E63]/60 font-semibold text-xs w-10"></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {pagosFiltrados.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-12">
                    <Wallet className="h-10 w-10 text-[#164E63]/15 mx-auto mb-2" />
                    <p className="text-sm text-[#164E63]/40">No se encontraron pagos</p>
                  </TableCell>
                </TableRow>
              ) : (
                pagosFiltrados.map((pago) => {
                  ...
                  return (
                    <TableRow key={pago.id} className="hover:bg-cyan-50/40 transition-colors cursor-pointer" onClick={() => setModalDetalle(pago)}>
                      <TableCell>
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-sm font-medium text-[#164E63] truncate max-w-[140px]">{pago.pacienteNombre}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm text-[#164E63]/70 truncate max-w-[200px]">{pago.concepto}</p>
                      </TableCell>
                      <TableCell className="text-right">
                        <span className={`text-sm font-bold ${pago.estado === "reembolsado" ? "text-red-500" : "text-[#164E63]"}`}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-[#164E63]/50">{metodoCfg.label}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <span className="text-xs text-[#164E63]/50">{formatFecha(pago.fechaPago)}</span>
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-7 w-7 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                              <MoreHorizontal className="h-4 w-4 text-[#164E63]/40" />
                            </Button>
                          </DropdownMenuTrigger>
        ...
        <DialogContent className="max-w-md">
          {modalDetalle && (() => {
            ...
            return (
              <>
                <DialogHeader>
                  <DialogTitle className="text-[#164E63]">Detalle del Pago</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div className="flex items-center gap-3 bg-gradient-to-r from-[#ECFEFF] to-white rounded-xl p-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#164E63]/40">{formatFecha(modalDetalle.fechaPago)}</p>
                    </div>
                    <div className="ml-auto text-right">
                      <p className={`text-xl font-bold ${modalDetalle.estado === "reembolsado" ? "text-red-500" : "text-[#164E63]"}`}>
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={item.label} className="flex justify-between items-start gap-4">
                        <span className="text-xs font-semibold text-[#164E63]/50 shrink-0">{item.label}</span>
                        <span className="text-sm text-[#164E63]/80 text-right">{item.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <DialogFooter className="gap-2 mt-4">
                  ...
                  <Button variant="outline" className="cursor-pointer text-[#164E63]">
      ...
      <Dialog open={modalRegistrar} onOpenChange={setModalRegistrar}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[#164E63]">Registrar Pago</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-[#164E63]/60">Monto</Label>
                <div className="relative">
                  <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#164E63]/30" />
                  <Input type="number" placeholder="0.00" className="pl-9 border-cyan-200" />
                </div>
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-[#164E63]/60">Método</Label>
                <Select>
                  <SelectTrigger className="border-cyan-200 cursor-pointer"><SelectValue placeholder="Método" /></SelectTrigger>
                  <SelectContent>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-[#164E63]/60">Concepto</Label>
              <Input placeholder="Ej: Membresía mensual, Sesión individual..." className="border-cyan-200" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-[#164E63]/60">Referencia Externa (opcional)</Label>
              <Input placeholder="Ej: SPEI-2026030101" className="border-cyan-200" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-[#164E63]/60">Fecha de Pago</Label>
              <Input type="date" className="border-cyan-200 cursor-pointer" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-[#164E63]/60">Notas (opcional)</Label>
              <textarea
                rows={2}
                className="w-full text-sm border border-cyan-200 rounded-xl px-3 py-2 resize-none focus:outline-none focus:border-[#0891B2] text-[#164E63] placeholder:text-[#164E63]/30"
          ...
          <DialogFooter className="gap-2 mt-4">
            ...
            <Button onClick={() => setModalRegistrar(false)} className="cursor-pointer bg-[#059669] hover:bg-[#059669]/90 text-white">
