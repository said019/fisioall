...
async function main() {
  ...
  const paola = await prisma.usuario.upsert({
    ...
    update: {
      ...
      bio: "Licenciada en Fisioterapia con certificación CONOCER ante la SEP. Fundadora de Kaya Kalp.",
    },
    create: {
      tenantId: tenant.id,
      nombre: "Paola",
  ...
  const gaby = await prisma.usuario.upsert({
    ...
    create: {
      ...
      passwordHash,
      rol: "fisioterapeuta",
      especialidades: ["Fisioterapia", "Rehabilitación Deportiva"],
      activo: true,
    },
  });
  ...
  const jenni = await prisma.usuario.upsert({
    ...
    create: {
      ...
      passwordHash,
      rol: "fisioterapeuta",
      especialidades: ["Tratamientos Faciales", "Tratamientos Corporales", "Masajes"],
      activo: true,
    },
  });
